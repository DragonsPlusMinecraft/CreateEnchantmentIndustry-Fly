...
public class ExperienceScene {
    public static void basic(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.world().modifyBlockEntity(util.grid().at(3, 2, 4), BrassTunnelBlockEntity.class,
                be -> be.getBehaviour(ServerSidedFilteringBehaviour.TYPE)
                        .setFilter(Direction.EAST, AllItems.EXP_NUGGET.getDefaultInstance()));
        scene.idle(5);
        scene.world().createItemOnBelt(util.grid().at(3, 1, 11), Direction.UP, AllItems.EXP_NUGGET.getDefaultInstance());
        scene.idle(20);
        scene.world().createItemOnBelt(util.grid().at(3, 1, 11), Direction.UP, AllItems.EXP_NUGGET.getDefaultInstance());
        scene.idle(20);
        scene.world().createItemOnBelt(util.grid().at(3, 1, 11), Direction.UP, AllItems.EXP_NUGGET.getDefaultInstance());
        ...
        scene.idle(5);
        scene.world().modifyBlockEntity(deployerPos, DeployerBlockEntity.class, be -> {
            be.heldItem = sword.copy();
            be.mode = DeployerBlockEntity.Mode.PUNCH;
        });
        scene.idle(5);
        ...
        scene.world().createItemOnBelt(deployerPos.west().below(), Direction.SOUTH, AllItems.EXP_NUGGET.getDefaultInstance());
        scene.idle(20);
    ...
    public static void prepare(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.world().modifyBlockEntity(
                ...
                be -> be.setHeldItem(new TransportedItemStack(CEIItems.EXPERIENCE_CAKE.asStack())));
        scene.idle(10);
        scene.overlay().showText(60)
        ...
        scene.world().modifyBlockEntity(
                util.grid().at(1, 1, 1),
                DepotBlockEntity.class,
                be -> be.setHeldItem(new TransportedItemStack(CEIItems.SUPER_ENCHANTING_TEMPLATE.asStack())));
        scene.idle(25);
        scene.overlay().showText(45)
